...
static struct p2p_srv_asp2 *find_key_value_record(
								struct p2ps_advertisement *adv_data,
								const char *key)
{
...

{
	...
	for (adv_data = p2p_get_p2ps_adv_list(wpa_s->global->p2p);
		...
		if (os_memcmp(adv_data->svc_name, svc, svc_len) == 0 &&
			    os_memcmp(adv_data->instance_name, instance_name, instance_len) == 0) {

			wpa_printf(MSG_DEBUG, "P2P: Service Found");

			if (info_len > 1) {
				wpa_printf(MSG_DEBUG, "P2P:Service Seeker required the values of specific keys");
				/*Extract the keys from Service Discovery Request and found the values*/
				...
				for (j = 1, str1 = ptr; ; j++, str1 = NULL) {
					...
					if (asrv == NULL) {
			 		...
			 		os_snprintf((buf + svc_res_len),2000,"%s=%s",asrv->key,asrv->value);
					svc_res_len  = svc_res_len + os_strlen(asrv->key) + os_strlen(asrv->value);
					svc_res_len++;
				}
				os_free(ptr);
			}
				...
				wpa_printf(MSG_DEBUG, "P2P: Service Seeker required  all <key=value> pair records.");
				if (dl_list_empty(&adv_data->p2p_srv_asp2)) {
					wpa_printf(MSG_DEBUG, "P2P: ASP2 Keys not found");
					wpas_sd_add_not_found(resp, P2P_SERV_ASP, srv_trans_id);
					return;
