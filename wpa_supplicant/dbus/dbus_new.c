...
/**
 * wpas_dbus_signal_p2p_provision_discovery_failure - Signals PD failure event
 *
 * @dev_addr - who sent the request or responded to our request.
 * @status - valid only in case of response
 * @adv_id -valid only in case of ASP Provision Discovery
 * @deferred_sessoin_resp - valid only in case of ASP Provision Discovery
 * Sends following provision discovery failure event
 */
				      ...
				      char *svc_str, char *info)
{
	...
	if (!wpa_dbus_dict_open_write(&iter, &dict_iter) ||
	    !wpa_dbus_dict_append_object_path(&dict_iter, "peer_object",
					      path) ||
	    !wpa_dbus_dict_append_uint32(&dict_iter, "srv_trans_id",
					      srv_trans_id) ||
	    !wpa_dbus_dict_append_uint32(&dict_iter, "adv_id",
					      adv_id) ||
			!wpa_dbus_dict_append_uint32(&dict_iter, "svc_status",
					      svc_status) ||
			!wpa_dbus_dict_append_uint16(&dict_iter, "config_methods",
					      config_methods) ||
...
static const struct wpa_dbus_method_desc wpas_dbus_interface_methods[] = {
	...
	{ "ASPProvisionDiscoveryRequest", WPAS_DBUS_NEW_IFACE_P2PDEVICE,
	  (WPADBusMethodHandler) wpas_dbus_handler_p2p_asp_prov_disc_req,
	  {
		  { "args", "a{sv}", ARG_IN },
		  END_ARGS
	  }
