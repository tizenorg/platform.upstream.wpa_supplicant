...

{
	...
	//TODO: Service Type can be NULL(To advertise a specific Unique Identifier). Need to be handle
	if (!p2p || !adv_str || !adv_str[0] || !cpt_priority)
		return -1;
	...
	//TODO: In case service type NULL, hash generation is defined by Instant Name. Need to be handle
	if (!p2ps_gen_hash(p2p, adv_str, buf))
		return -1;
	...
	//TODO: As per ASP 2.0 spec,Service Type could be null so we may need to modify this structure
	tmp = p2p->p2ps_adv_list;
	prev = &p2p->p2ps_adv_list;
	while (tmp) {
		if (tmp->id == adv_data->id) {
